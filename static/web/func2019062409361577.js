...
document.getElementById("downloada").addEventListener("click", function () {
      var value  = document.getElementById("lin").outerHTML;
      exportRaw("nicetool.svg",value);
...
document.getElementById('report_mode').addEventListener("click", function() {
  ...
  $('#reportTools').show(200, function() {
    document.getElementById('reportTotalSurface').innerHTML = "总面积 : <b>"+(globalArea/3600).toFixed(1)+ "</b> m²";
    $('#reportTotalSurface').show(1000);
    document.getElementById('reportNumberSurface').innerHTML = "房间数 : <b>"+ROOM.length+ "</b>";
    $('#reportNumberSurface').show(1000);
    var number = 1;
    var reportRoom = '<div class="row">\n';
    for (var k in ROOM) {
      var nameRoom = "区域"+number+" <small>(未标注)</small>";
    ...
    reportRoom+='<p>开关数量 : '+switchNumber+'</p>';
    reportRoom+='<p>插座数量 : '+plugNumber+'</p>';
    reportRoom+='<p>灯光数量 : '+lampNumber+'</p>';
    reportRoom+='</div>';
    reportRoom+='<div>\n';
    //reportRoom+='<h2>明细</h2>\n';
    var number = 1;
    ...
    for (var k in ROOM) {
      reportRoom+= '<div class="row" style="display:none;">\n';
      var nfc = true;
      var nameRoom = "Pièce n°"+number+" <small>(sans nom)</small>";
      if (ROOM[k].name != "") nameRoom = ROOM[k].name;
      reportRoom+= '<div class="col-md-4"><p>'+nameRoom+'</p></div>\n';
...
window.addEventListener("load", function(){
  ...
  if (!localStorage.getItem('history')) $('#recover').html("<p>选择一个模板");
  $('#myModal').modal();
});
...
$('#node_mode').click(function() {
    $('#boxinfo').html('修改墙面<br/><span style=\"font-size:0.7em\">提示：切割房间的墙壁可以取消其配置</span>');
    fonc_button('node_mode');
});

$('#text_mode').click(function() {
    $('#boxinfo').html('添加文字<br/><span style=\"font-size:0.7em\">将光标移动到所需位置并键入文本。</span>');
